...
export function renderLounges(s) {
  ...
  const inferredTerminals = loungeFlightAirline && loungeAirport
    ...
    : [];
  // Combine — flight-on-file wins (more specific), else fall back to inferred
  const combinedTerminalSet = new Set([...flightTerminals, ...inferredTerminals]);
  ...
  // Pretty terminal label list, e.g. ["Terminal 4"], ["Terminal A", "Terminal B"]
  const terminalLabels = [...combinedTerminalSet].map(t => t.match(/^[A-Z0-9]+$/) ? `Terminal ${t}` : t);

  const loungesWithAccess = lounges.map(l => ({ ...l, _access: getLoungeAccess(l.network, l), _inYourTerminal: isInYourTerminal(l) }));
          ...
          <div style={{ background: dv.paper, border: `1px solid ${dv.cream}`, borderLeft: `3px solid ${dv.accent}`, padding: isMobile ? "14px 16px" : "16px 22px", marginBottom: 24, display: "flex", flexDirection: isMobile ? "column" : "row", gap: 16, alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between" }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontFamily: dv.serif, fontSize: 16, color: dv.ink, lineHeight: 1.3 }}>
                {fromFlight
                  ? <>Your terminal: <strong style={{ color: dv.accent, fontWeight: 500 }}>{terminalLabels.join(" · ")}</strong>. Showing lounges in your terminal only.</>
                  : <>{airlineName || "This airline"} operates from <strong style={{ color: dv.accent, fontWeight: 500 }}>{terminalLabels.join(" · ")}</strong>. Showing lounges there only.</>
                }
              </div>
              {!fromFlight && (
            ...
            <button onClick={() => setLoungeShowAllTerminals?.(!loungeShowAllTerminals)}
              ...
              {loungeShowAllTerminals ? `All terminals · ${loungesWithAccess.length}` : `Same terminal · ${visibleLounges.length} of ${loungesWithAccess.length}`}
            </button>
          </div>
        );
      })()}
